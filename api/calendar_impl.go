...
func (s Server) GetAPICalendarMe(w http.ResponseWriter, r *http.Request, params GetAPICalendarMeParams) {
	...
	if err != nil {
		s.Logger.Error("failed to create msgraph client, ", zap.String("request ID", reqUUID), zap.Error(err))
	...
	if err != nil {
		s.Logger.Error("failed to make calendar me msgraph api call, ", zap.String("request ID", reqUUID), zap.Error(err))
...
func (s Server) PostAPICalendarMe(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&eventRequest); err != nil {
		s.Logger.Error("failed to parse event body, ", uuidStr, zap.Error(ErrUnmarshalBody), zap.Error(err))
	...
	if err != nil {
		s.Logger.Error("failed to create calendar event, ", uuidStr, zap.Error(err))
	...
	if err != nil {
		...
		s.Logger.Error("failed to send notification, ", uuidStr, zap.Error(err))
	}
